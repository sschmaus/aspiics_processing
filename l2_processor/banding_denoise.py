...
Banding noise correction functions for ASPICIS L2 processing.

Copyright (c) 2026 Simeon Schmauß
SPDX-License-Identifier: MIT

Author: Simeon Schmauß <simeon.schmauss@gmail.com>
"""
...
def fast_nanmedian(img, size, *args, **kwargs):
    """Apply a windowed median filter to an image, handling NaN values appropriately.
...
def _calculate_banding_from_reference(img_ref, filter_1d='global', axis=0):
    """Correct banding along the first dimension in an image using specified filtering and averaging methods.
    ...
    filter_1d : int | 'global', optional
        Size of the median filter applied in 1D to remove outliers.
        Default is 4 * filter_2d.
        If 'global', no 1D filtering is applied and banding profile is computed for the entire line with np.nanmedian.
...
def _integer_downsample(img, factor):
    """Downscale image by integer factor using pixel averaging."""
    if factor == 1:
        return img
    elif factor < 1:
...
def _generate_banding_reference(img, ref_method="median", filter_2d=15, ref_downscale=1, structures_threshold=None):
    """
    Generate a reference image for banding profile calculation.
    ...
    Returns
    -------
    banding_ref : 2D numpy array
        The generated reference image for banding profile calculation.
        The same size as the input image regardless of downscaling.

    """

    banding_ref = np.float32(img.copy())

    if structures_threshold is None:
        # set default threshold for remaining structures filtering
    ...
    if ref_downscale > 1:
        ...
        banding_ref = _integer_downsample(banding_ref, ref_downscale)
    ...
    if ref_method == "median":
        ...
        img_med = fast_nanmedian(banding_ref, size=filter_2d, mode='constant')
        banding_ref = banding_ref - img_med
        ...
        img_ref_denoised = fill_nan_mirror(banding_ref)
        ...
        banding_ref = banding_ref - img_ref_denoised

    if structures_threshold is not False:
        # zero-out remaining structures above a certain amplitude
        std_ref = np.nanstd(banding_ref)
        remaining_structures = np.abs(banding_ref) > std_ref * structures_threshold
        ...
        banding_ref[remaining_structures] = np.nanmedian(banding_ref)

    if ref_downscale > 1:
        # upscale reference image back to original size
        banding_ref = _integer_upsample(banding_ref, ref_downscale)

    return banding_ref


def correct_banding(img, filter_2d=15, apply_vert='global', apply_horiz='global', ref_method="median", ref_downscale=1, structures_threshold=None, split_rows=False, plotting=False):
    """
    Correct banding along the first dimension in an image using specified filtering and averaging methods.
    ...
    filter_2d : int, optional
        Size of the median filter applied in 2D to remove large structures.
    apply_vert : str, int, optional
        Method to apply vertical banding correction. Options are "global" (default), int (median filter size), or False (no vertical correction).
    apply_horiz : str, int, optional
        Method to apply horizontal banding correction. Options are "global" (default), int (median filter size), or False (no horizontal correction).
    ref_method : str, optional
        Method to use for reference image generation. Options are "median" or "nlmeans".
    ref_downscale : int, optional
        Factor by which to downscale the reference image for banding profile calculation for faster processing.
    ...
    # calculate vertical banding profile, accounting for separate row processing and reference downscaling
    if apply_vert is not False:
        if not split_rows or ref_downscale > 1:
            # if downscaling is used, process all rows together because row separation is lost
            banding_vert = _calculate_banding_from_reference(banding_ref, filter_1d=apply_vert, axis=0)
        else:
            banding_vert = np.zeros_like(img)
            if apply_vert != 'global':
                apply_vert = max(3, apply_vert // 2)
            banding_vert_odd = _calculate_banding_from_reference(banding_ref[0::2, :], filter_1d=apply_vert, axis=0)
            banding_vert_even = _calculate_banding_from_reference(banding_ref[1::2, :], filter_1d=apply_vert, axis=0)
    ...
    if apply_horiz is not False:
        banding_horiz = _calculate_banding_from_reference(banding_ref, filter_1d=apply_horiz, axis=1)
    ...
    if plotting:
        subtext = f"Reference method: {ref_method}, 2D filter size: {filter_2d}, 1D filter sizes (vert, horiz): {apply_vert}, {apply_horiz}, Split rows: {split_rows}, Reference downscale: {ref_downscale}"
        wd = (256,768,256,768)
        # banding_ref = _integer_upsample(banding_ref, ref_downscale)
        _plot_banding_correction_reference(img, banding_ref, banding, split_rows, wd, title=f"{ref_method} banding correction", subtext=subtext)
    
    return corrected_img

def correct_banding_multiscale(img, ref_method ="median", layers_2d=[3,7,15], apply_vert='global', apply_horiz='global', ref_downscale=None, split_rows=False, plotting=True):
    """
    ...
    layers_2d : int, optional
        ...
        Default is [3,7,15]
    apply_vert : list of int | str, optional
        List of 1D filter sizes for vertical banding correction corresponding to each 2D layer.
        Default is "global", which applies a global filter size.
    apply_horiz : list of int | str, optional
        List of 1D filter sizes for horizontal banding correction corresponding to each 2D layer.
        Default is "global", which applies a global filter size.
    ...
    if not isinstance(layers_2d, list):
        raise ValueError("layers_2d must be a list of integers.")
    if not isinstance(apply_vert, list):
        apply_vert = [apply_vert for _ in layers_2d]
    if not isinstance(apply_horiz, list):
        apply_horiz = [apply_horiz for _ in layers_2d]
    
    if len(apply_vert) != len(layers_2d):
        raise ValueError("apply_vert list length must match layers_2d")
    if len(apply_horiz) != len(layers_2d):
        raise ValueError("apply_horiz list length must match layers_2d")
    
    if ref_downscale is None or ref_downscale == 1:
        ref_downscale = [1 for _ in layers_2d]
    if len(ref_downscale) != len(layers_2d):
        raise ValueError("ref_downscale list length must match layers_2d")
    ...
    if plotting:
        # plotting for debugging
        subtext = f"Reference method: {ref_method}, 2D filter sizes: {layers_2d}, 1D filter sizes (vert, horiz): {apply_vert}, {apply_horiz}, Split rows: {split_rows}, Reference downscale: {ref_downscale}"
        wd = (256,768,256,768)

        # re-generate banding reference at final scale for plotting
        print("Generating banding reference at largest scale for plotting (may take a while because downscaling is not used)...")
        img_ref = _generate_banding_reference(img, ref_method, layers_2d[-1], ref_downscale[1])

        _plot_banding_correction_reference(img, img_ref, img-corrected_img, split_rows, wd, title=f"{ref_method} banding correction", subtext=subtext)
...
def correct_banding_splitrows(img, *args, **kwargs):
    ...
    See Also
    --------
    correct_banding : Main banding correction function with split_rows option.
    """
    img = img.copy()
...
def correct_banding_splitrows_multiscale(img, *args, **kwargs):
    ...
    See Also
    --------
    correct_banding_multiscale : Main banding correction function with split_rows option.
    """
    img = img.copy()
...
def plot_banding_correction_before_after(img_orig, img_corrected, split_rows=False, window=(256,768,256,768), title="", subtext="", dpi=100):
    ...
    pct99_banding = np.nanpercentile(np.abs(banding_profile), 99)
